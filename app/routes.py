...
def candidate():
    ...
    candidatehistory = db.session.query(candidate_history).order_by(desc(candidate_history.id)).all()
...
def members():
    ...
    memberhistory = db.session.query(member_history).order_by(desc(member_history.id)).all()

    return render_template('members.html', members_data=members_data,memberhistory=memberhistory)
        ...
        try:
            if id is not None:
                ...
                if old_member:
                    ...
                    if password:
                         changes['password'] = {'from': old_member.password, 'to': password}
                ...
                if not email_exists:
                    ...
                    if 'user_id' in session:
                        ...
                        user_name = db.session.query(Users).filter_by(id=user_id).first()
                        current_time = datetime.utcnow().strftime("%b %d, %Y %I:%M%p")
                        user_statement = f"New Member named {fname} {lname} was created by {user_name.fname} {user_name.lname} ({current_time}) "
                        print(user_statement)
        ...
        if position == 'select' or not position:
         ...
         if formid is not None:
            ...
            if status == "Candidate Placement":
                ...
                status = 'Resume Sent'
                user_statement = f"Resume is sent  of Candidate  ({candidate}) for {entry.company} for {entry.position} by {name} ({current_time}) "
            print(user_statement)
            ...
            if id is not None:
                ...
                if old_job_order:
                    ...
                    if changes:
                        ...
                        if change_statements:
                            current_time = datetime.utcnow().strftime("%b %d, %Y %I:%M%p")  # Format current time
                            user_statement = f"{user_name.fname} {user_name.lname} changed " + " and ".join(
                                change_statements) + f" ({current_time})"
                            print(user_statement)
    ...
    if request.method == 'POST':
        ...
        if id is not None:
                    ...
                    if changes:
                        ...
                        for field, values in changes.items():
                            ...
                            if field == 'description' or field == 'responsibility' or field == 'eligibility' or field == 'notes':
                                ...
                                if values['to'] == "123":
                                    values['to'] = "Geox hr, Hands hr, i8is"
                                elif values['to'] == "12":
                                    values['to'] = "Geox hr, Hands hr"
                                elif values['to'] == "23":
                                    values['to'] = "i8is, Hands hr"
                                elif values['to'] == "13":
                                    values['to'] = "i8is, Geox hr"
                                elif values['to'] == "1":
                                    values['to'] = "Geox hr"
                                elif values['to'] == "2":
                                    values['to'] = "Hands hr"
                                ...
                                if values['from'] == "123":
                                    values['from'] = "Geox hr, Hands hr, i8is"
                                elif values['from'] == "12":
                                    values['from'] = "Geox hr, Hands hr"
                                elif values['from'] == "23":
                                    values['from'] = "i8is, Hands hr"
                                elif values['from'] == "13":
                                    values['from'] = "i8is, Geox hr"
                                elif values['from'] == "1":
                                    values['from'] = "Geox hr"
                                elif values['from'] == "2":
                                    values['from'] = "Hands hr"
                                elif values['from'] == "3":
                                    values['from'] = "i8is"
                                else:
                                    values['from'] = ""
                        ...
                        if change_statements:
                            if 'user_id' in session:
                                ...
                                user_statement = f"{user_name.fname} {user_name.lname} made the following changes:" + " and ".join(change_statements) + f" ({current_time})"
                                print(user_statement)
            ...
            if 'user_id' in session:
                        ...
                        user_name = db.session.query(Users).filter_by(id=user_id).first()
                        current_time = datetime.utcnow().strftime("%b %d, %Y %I:%M%p")
                        user_statement = f"This Job name {title} was created by {user_name.fname} {user_name.lname} for {selected_companies}  ({current_time}) "
                        print(user_statement)
...
def editjoborder(id=None):
    # Retrieve job order data corresponding to the provided id
    job_data = Joborder.query.filter(Joborder.id == id).first()
